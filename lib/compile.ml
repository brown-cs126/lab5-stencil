...
let function_label_mod_pair s =
  let nasm_char c =
    ...
    | _ ->
        '_'
  in
  Printf.sprintf "function_mod_pair_%s_%d" (String.map nasm_char s)
    (Hashtbl.hash s)
...
let rec compile_expr (defns : defn list) (tab : symtab) (stack_index : int)
    (is_tail : bool) : s_exp -> directive list = function
  | Num x ->
  ...
  | Lst (Sym f :: args) as e when is_defn defns f && is_tail ->
...
let compile (exps : s_exp list) =
  ...
  @ [Label "lisp_entry"]
  @ compile_expr defns Symtab.empty (-8) true body
  @ [Ret]
